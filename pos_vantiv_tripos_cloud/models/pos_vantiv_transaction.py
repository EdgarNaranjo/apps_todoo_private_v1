...
class PosVantivTransaction(models.Model):
    ...
    lane_id = fields.Many2one(comodel_name='pos_vantiv_tripos_cloud.lane', string=_('Lane'))
    ...
    def do_transaction(self, data):
        ...
        body = {
            'laneId': lane.lane_id,
            'transactionAmount': data.get('amount_total'),
        ...
        try:
            r = requests.post(url=base_url, headers=headers, data=json.dumps(body), timeout=(6, 12))
            r.raise_for_status()
            response = r.json()
            print(response)
            self.create({
            ...
            _logger.info('Vantiv URL {} - Status {} Vantiv status {} Line {}'.format(base_url, r.status_code, response.get('statusCode', 'No status'), lane.name))
        except Timeout:
            response = "timeout"
        return response
