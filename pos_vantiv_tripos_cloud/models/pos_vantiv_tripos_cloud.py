...
class PosVantivTriposCloudLane(models.Model):
    ...
    activation_code = fields.Char(
        string='Activation Code', required=True, help='Pin Pads Activation Code'
    )
    ...
    def _compute_get_name(self):
        for rec in self:
            rec.name = '{} - {}'.format(rec.lane_id, rec.description)
    ...
    def add_lane_to_triposcloud(self):
        ...
        for rec in self:
            ...
            req = requests.post(
                base_url, headers=headers, data=json.dumps(data)
            )
