    ...
    def check_material_line_in_task(self):
        ...
        for record in self:
            ...
            if record.state == 'done':
                if record.task_id and record.picking_type_id.code == 'incoming' and not record.picking_type_id.return_picking_type_id:
    ...
    def _action_send_mail(self, auto_commit=False):
        if self.model == 'project.task':
            ...
            if self.env.context.get('default_res_id'):
                task_id = self.env.context.get('default_res_id')
                obj_task_id = self.env['project.task'].search([('id', '=', task_id)], limit=1)
                if obj_task_id:
                    materials = self.env['ot.material.line'].search([('task_id', '=', obj_task_id.id), ('check_process', '=', True)])
                    if materials:
                        materials.write({'check_send': True})
                    operations = self.env['operation.line'].search([('task_id', '=', obj_task_id.id), ('check_process', '=', True)])
                    if operations:
                        operations.write({'check_send': True})
                    obj_task_id.count_doc_sign += 1
