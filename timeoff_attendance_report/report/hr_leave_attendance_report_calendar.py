...
class LeaveAttendanceReportCalendar(models.Model):
    ...
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'hr_leave_attendance_report_calendar')
        self.env.cr.execute("""CREATE OR REPLACE VIEW hr_leave_attendance_report_calendar AS
        (SELECT
    ...
    @api.depends('on_holiday', 'on_attendance')
    def _get_color(self):
        for record in self:
            record.color = 1
            if record.on_holiday:
                record.color = 9
            if record.on_attendance:
                record.color = 2
