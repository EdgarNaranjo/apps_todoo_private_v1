...
class RepairOrder(models.Model):
    ...
    def _compute_location_id(self):
        ...
        for order in self:
            ...
            if not location and order.picking_type_id:
                location = order.picking_type_id.default_location_src_id
            order.location_id = location

    @api.depends('picking_type_id')
    def _compute_product_location_src_id(self):
        param_id = self.env['ir.config_parameter'].sudo().get_param('order_repair_todoo.product_location_src_id')
        for order in self:
            location = False
            if param_id and str(param_id).isdigit():
                location_candidate = self.env['stock.location'].browse(int(param_id))
                if location_candidate.exists() and (
                    ...
                    location = location_candidate
            if not location and order.picking_type_id:
                location = order.picking_type_id.default_product_location_src_id
            order.product_location_src_id = location
