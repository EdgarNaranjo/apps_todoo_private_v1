...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        values_to_set = {
            'order_repair_todoo.repair_type_id': self.repair_type_id.id,
            'order_repair_todoo.reason_id': self.reason_id.id,
            'order_repair_todoo.invoice_method': self.invoice_method,
            'order_repair_todoo.location_id': self.location_id.id,
            'order_repair_todoo.check_demo': '1' if self.check_demo else '0'
        }
    ...
    def get_values(self):
        ...
        config_params = self.env['ir.config_parameter'].sudo()
        res.update({
            'check_demo': config_params.get_param('order_repair_todoo.check_demo') == '1'
        })
        return res
