...
class Employee(models.Model):
    ...
    def check_timesheet_cost_change(self):
        env_cost_history = self.env['employee.cost.history']
        for record in self:
            if record.timesheet_cost:
                obj_cost_by_employee = env_cost_history.search([('employee_id', '=', record.id)])
                if obj_cost_by_employee:
                    filter_check = obj_cost_by_employee.filtered(lambda e: e.current_cost == record.timesheet_cost)
                    filter_no_check = obj_cost_by_employee.filtered(lambda e: e.current_cost != record.timesheet_cost)
                    if filter_check:
...
class EmployeeCostHistory(models.Model):
    ...
    def onchange_employee_id(self):
        ...
        if obj_employee:
            ...
            self.previous_cost = obj_employee.timesheet_cost
            if self.rate_start:
