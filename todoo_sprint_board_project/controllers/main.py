...
from odoo.addons.todoo_sprint_board_project.models.sprint_board import SprintBoard as _SprintBoardModel
...
class SprintBoardController(http.Controller):
    ...
    @staticmethod
    def _is_bug(task):
        """Delega en SprintBoard._is_bug_task para evitar lógica duplicada."""
        return _SprintBoardModel._is_bug_task(task)
        ...
        if kpi_filter == "bugs":
            ...
            def _is_at_risk(b):
                if not b.sprint_id.end_date:
                    return False
                days = max(0, (b.sprint_id.end_date.date() - _today).days)
