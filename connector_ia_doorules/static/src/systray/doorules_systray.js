...
export class DooRulesSystray extends Component {
    ...
    async _onClick() {
        // En Odoo 18, intentamos obtener el nombre de múltiples fuentes de forma jerárquica
        const user = this.env.services.user;
        const sessionInfo = window.odoo && window.odoo.session_info;
        
        // Probamos: nombre del servicio, login del servicio, o datos de sesión global
        const userName = (user && (user.name && user.name !== "Odoo User" ? user.name : user.login)) || 
                         (sessionInfo && (sessionInfo.name || sessionInfo.username || sessionInfo.partner_name)) || 
                         (this.env.session && this.env.session.name) || 
                         "Edgar87";
        ...
        if (controller) {
            // Detección profunda de modelo para Odoo 18 (props, action, o sub-componentes)
            let resModel = (controller.props && (controller.props.resModel || controller.props.model)) || 
                           (controller.action && controller.action.res_model) ||
                           (controller.props && controller.props.action && controller.props.action.res_model);
            
            // Si el controlador es una vista de búsqueda o similar, a veces el modelo está en searchModel
            if (!resModel && controller.searchModel) {
                resModel = controller.searchModel.resModel;
            }
