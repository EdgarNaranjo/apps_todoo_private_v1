...
class ConstructorPowerBiData(models.Model):
    ...
    def write(self, vals):
        ...
        if 'model_id' in vals:
            for record in self.filtered(lambda r: r.type_query == 'simple'):
                ...
                if obj_create_item:
                    _logger.info('Create items by WRITE (super)')
        return res
    ...
    def create_document(self, file_excel, filename, constructor_id):
        ...
        folder = self.env.ref('powerbi_todoo.documents_document_power_bi')
...
class PowerBiSetting(models.Model):
    ...
    def create(self, vals_list):
        ...
        if active_power_bi:
            line_items = []
            for req in requests:
                existing_power_bi_ids = req.line_ids.mapped('power_bi_id')
                for item in active_power_bi:
                    if item not in existing_power_bi_ids:
                        line_items.append({
                            'setting_id': req.id,
                            'power_bi_id': item.id,
    ...
    def update_list_excel(self, wizard=False):
        ...
        is_wizard_filter = origin_view == 'wizard_filter'
        if not is_wizard_filter:
            cron = self.env.ref('powerbi_todoo.ir_cron_data_update_list_csv')
            if cron.numbercall <= 0:
                cron.write({'active': False})
                return
            cron.write({'numbercall': cron.numbercall - 1})
        state_active = [('state', '=', 'active')]
        domain = state_active + ([('id', '=', wizard.setting_id.id)] if is_wizard_filter else [])
        setting_ids = self.env['power.bi.setting'].search(domain)
        for setting in setting_ids:
            if is_wizard_filter:
                wizard_line_ids = wizard.line_ids.ids
                list_line = setting.line_ids.filtered(lambda e: e.document_id and e.id in wizard_line_ids).sorted('size_data')
            else:
                list_line = setting.line_ids.filtered(lambda e: e.document_id and not e.ok_process).sorted('size_data')
                if setting.qty_partition:
                    list_line = list_line[:int(setting.qty_partition)]
            if list_line:
                for line in list_line:
                    self.update_file_excel(line)
                list_line.write({'ok_process': True})
