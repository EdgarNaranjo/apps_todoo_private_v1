...
class HelpdeskTicket(models.Model):
    ...
    days_open = fields.Integer(
        ...
        store=False,
        help='Number of days since the ticket was created',
    )
    is_overdue = fields.Boolean(
        string='Overdue',
        compute='_compute_days_open',
        store=False,
        help='Ticket open for more than 5 days without resolution',
    )
    ...
    @api.depends('create_date', 'closed')
    def _compute_days_open(self):
        for ticket in self:
            if ticket.create_date:
                delta = fields.Datetime.now() - ticket.create_date
                ticket.days_open = delta.days
                ticket.is_overdue = delta.days > 5 and not ticket.closed
            else:
                ticket.days_open = 0
    ...
    def copy(self, default=None):
        ...
        res = super().copy_data(default=default)[0]
        return res
...
class Task(models.Model):
    ...
    def onchange_timesheet_ids(self):
        if self.timesheet_ids and self.project_id:
            hour_p = self.planned_hours - sum(self.timesheet_ids.mapped('unit_amount'))
            self.planned_hours = hour_p
