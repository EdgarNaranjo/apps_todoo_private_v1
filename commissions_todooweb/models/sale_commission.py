    ...
    def action_commission(self):
        for obj_commision in self:
            ...
            if commission_type and commission_calc and obj_commision.commercial_id and obj_commision.date_to and obj_commision.date_from:
                invoices_ids = self.env['account.invoice'].search([('state', 'not in', ['draft', 'cancelled']), ('type', 'in', ['out_invoice', 'out_refund'])])
                invoices_filtered = invoices_ids.filtered(lambda e: obj_commision.date_from <= e.date_invoice <= obj_commision.date_to)
                ...
                if invoices_filtered:
                    for invoice in invoices_filtered:
                        ...
                        if invoice.type == 'out_refund':
                            invoices_id = self.env['account.invoice'].search([('number', '=', invoice.origin)], limit=1)
                            if invoices_id:
                                origin = invoices_id.origin
                                factor = -1
                        if factor != 0:
                            if origin:
                                _logger.info('Invoice %s, origin %s' % (invoice.number, origin))
